...
export default function Testimonials() {
  return (
    <section className="rv border-y border-[var(--line)] bg-[var(--color-panel)]">
      <div className="mx-auto max-w-[1120px] px-6 py-24">
        ...
        <div className="mt-9 grid gap-[18px] sm:grid-cols-2 lg:grid-cols-3">
          {QUOTES.map((t) => (
            <figure
              key={t.co}
              className="rounded-2xl border border-[var(--line)] bg-[#0A0C0F] p-6"
            >
              <div className="mb-5 grid h-9 w-[110px] place-items-center rounded-md border border-dashed border-white/15 text-[12px] text-white/35">
                {t.co}
              </div>
              <blockquote className="text-[15.5px] leading-relaxed text-white/85">
                «{t.q}»
              </blockquote>
